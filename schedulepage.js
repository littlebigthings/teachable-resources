class ANIMATETABS {
    constructor(tabElem) {
        ...
        this.toOpenCloseWrp = this.tabItem.querySelector("[wrapper='panel-content']");
    ...
    resetTab(target) {
        if (target != undefined || target != null) {
            ...
            let getAllcontentTabs = target.parentElement.querySelectorAll("[wrapper='panel-content']");
    ...
    addItemsToArray() {
        if ((this.eventWrapper != undefined || this.eventWrapper != null) && this.eventItems.length != 0) {
            this.eventItems.forEach(item => {
                let elementToSetHeight = item.querySelector("[wrapper='panel-content']");
